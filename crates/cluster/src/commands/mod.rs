...
pub(crate) const MAX_NODE_OPERATOR_DATA_BYTES: u16 = 4096;
pub(crate) const DEFAULT_EVENT_PROPAGATION_LATENCY: u64 = 5;
pub(crate) const DEFAULT_CLOCK_SKEW: u64 = 500;
...
impl SharedArgs {
    pub(crate) async fn provider(&self) -> anyhow::Result<RpcProvider> {
        let signer = Signer::try_from_private_key(&self.signer_key)?;
    ...
    /// provider URL.
    pub(crate) async fn new_client(&self) -> anyhow::Result<Cluster<ClusterConfig>> {
        ...
        let connector = RpcProvider::new(provider_url, signer).await?;

        // let sc = provider.connect(address).await?;

        let cfg = ClusterConfig {
            encryption_key: self.encryption_key.parse()?,
        };

        let client = Cluster::connect(cfg, &connector, address).await?;
...
fn parse_operators_from_str(operators_str: &str) -> anyhow::Result<Vec<NodeOperator>> {
    ...
    let operators = operators
        .into_iter()
        .map(|op| {
            // TODO: refactor this expect unwrap
            let id = op
                .id
                .parse()
                .expect("operator ID is not a valid hex encodedd string");
            NodeOperator::new(id, op.name, op.nodes, op.clients)
        })
        .try_collect()?;
